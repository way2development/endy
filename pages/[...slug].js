...
const Page = ({ data, locale, selectedSaleId, isLastChance, enFrSlugsIds }) => {
  ...
  useEffect(() => {
    ...
    const translatedRoute = getTranslatedRouteById(otherPagesData)

    const redirectOtherPagesData = {
      localizedRoutes,
      locale,
    ...
    assignLocalizedRoutes(redirectOtherPagesData)
  }, [router])
...
export async function getStaticProps({ params, locale, preview }) {
  ...
  const enFrSlugsIds = await getAllDocSlugsIds('page')
